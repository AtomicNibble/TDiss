...
TEST(Misc, NotEnougthSpace)
{
	CodeStream strm(0x0, pOps, sizeof(pOps), DisOptions::SKIP_INVALID, CodeType::CODE_64BIT);

	// The byte code has 217 instructions
	Instruction instructions[200];
	size_t instructionCount;

	DisResult::Enum result = Diss::disassemble(strm, instructions, 200, &instructionCount);

	ASSERT_EQ(DisResult::OUTDATAFULL, result);
	ASSERT_EQ(200_sz, instructionCount);
}

TEST(Misc, VectorOutput)
{
	CodeStream strm(0x0, pOps, sizeof(pOps), DisOptions::SKIP_INVALID, CodeType::CODE_64BIT);
	...
	ASSERT_EQ(0xd9_sz, instructions.size());
