...
#define TDISS_LOG0(pFmt, ...) if (pLogger_) { pLogger_->Log(IDissLogger::Verbosity::LVL0, pFmt, __VA_ARGS__); }
#define TDISS_LOG1(pFmt, ...) if (pLogger_) { pLogger_->Log(IDissLogger::Verbosity::LVL1, pFmt, __VA_ARGS__); }
#define TDISS_LOG2(pFmt, ...) if (pLogger_) { pLogger_->Log(IDissLogger::Verbosity::LVL2, pFmt, __VA_ARGS__); }

#define TDISS_WARN(pFmt, ...) if (pLogger_) { pLogger_->Warning(pFmt, __VA_ARGS__); }
#define TDISS_ERR(pFmt, ...) if (pLogger_) { pLogger_->Error(pFmt, __VA_ARGS__); }
