...
namespace TDiss
{
	...
	CodeType::Enum Diss::GetEffectiveOperandSize(uint32_t prefixes, uint32_t instructionFlags, uint32_t rex) const
	{
		...
		if (bitUtil::IsBitFlagSet(prefixes, InstructionFlag::PRE_OP_SIZE)) {
	...
	CodeType::Enum Diss::GetEffectiveAddressSize(uint32_t prefixes) const
	{
		...
		if (bitUtil::IsBitFlagSet(prefixes, InstructionFlag::PRE_ADDR_SIZE)) {
	...
	const InstInfo* Diss::instLookupPreFixed(PrefixState& ps, InstNode in) const
	{
		...
		if (checkOpSize) {
			...
			if (!bitUtil::IsBitFlagSet(flags, InstructionFlag::PRE_OP_SIZE)) {
				return nullptr;
			}
		}
		...
		else if (instType == NodeType::DIVIDED) // 2 bytes + divided range
		{
...

			{
				...
				if (pInfo) {
					...
					if (bitUtil::IsBitFlagSet(flags, InstructionFlag::NOT_DIVIDED)) {
						return pInfo;
					}
				}
